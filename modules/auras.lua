...
local function scan(parent, frame, type, config, displayConfig, filter)
	...
	while( true ) do
		index = index + 1
		local name, texture, count, auraType, duration, endTime, caster, isRemovable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff = C_UnitAuras.GetAuraDataByIndex(frame.parent.unit, index, filter)
