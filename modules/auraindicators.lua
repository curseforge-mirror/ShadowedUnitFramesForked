...
local function scanAuras(frame, filter, type)
	...
	while( true ) do
		index = index + 1
		local name, texture, count, auraType, duration, endTime, caster, isRemovable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff = C_UnitAuras.GetAuraDataByIndex(frame.unit, index, filter)
